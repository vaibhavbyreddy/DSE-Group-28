...
Taking all of the above into consideration, the following criteria were selected: \textbf{Cost, Versatility, Performance, Reliability, and Sustainability}. 
...
\textbf{Cost:} a constraint, but not a mission-critical one. A configuration that is very expensive is painful, but doesn't make the aircraft scientifically invalid.
...
Assigning weights from 1 to 9 to the n criteria, the PCM is constructed so that the rows represent the ratios of a criterion's score to the scores of the other criteria \cite{Saaty 1980}. The obtained matrix is shown below.
