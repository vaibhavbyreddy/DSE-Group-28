...
\autoref{tab:Final_Candidate_Architectures} lists the design configurations considered for the trade-off. Additionally, each design option features a static port for the tail, fuselage-mounted fuel tanks and fuselage-mounted engines \footref{url:baseline_report}. The low-wing configuration now incorporates a static wing port, as it was found that a variable wing port would significantly compromise landing gear integration and reduce overall stability.
...
As the changes between considered configurations lie mostly in the modularity details, a separate trade-off will be performed concurrently to determine the most suitable materials for different subsystems of the final configuration. Candidate will be discussed in \autoref{sec:Material_Selection}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/Sketches/HUG-CFG-301.jpg}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/Sketches/HUG-CFG-302.jpg}
        \caption{HUG-CFG-302: low wing, variable wing port, no canard port}
        \label{fig:hug-cfg-302}
    \end{subfigure}
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/Sketches/HUG-CFG-303.jpg}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/Sketches/HUG-CFG-304.jpeg}
...
Meaningful trade-off criteria should be developed such that all available technical specifications and key performance metrics are considered. Moreover, it is recommended to identify criteria where one or more configurations perform poorly, as these are often the factors that distinguish them. In contrast, using criteria where all configurations score similarly is less useful, since such criteria do not provide meaningful differentiation \cite{SMAD}. Lastly, the key requirements can be used to ensure that the developed criteria encompass all crucial design aspects.
...
\hline
\textbf{Criterion} & \textbf{Rationale for criteria selection} \\ 
\hline
\endfirsthead

\hline
\textbf{Criterion} & \textbf{Rationale for selecting criterion} \\ 
\hline
\endhead
...
Versatility & HUG-CFG-301 is a low-wing concept that prevents the testing of truss-braced wing concepts, which is considered a stakeholder requirement specified by NLR \footnote{In conversation with NLR}. Similarly, HUG-CFG-302 prevents the testing of canard-dependent configurations, which limits the range of possible test scenarios.\\ \hline

Reliability & Static canard ports introduce an additional potential failure mode. In addition, the variable wing ports in HUG-CFG-304 increase fatigue exposure at modular interfaces, which raises operational risk. Furthermore, the HUGO design must comply with specific reliability requirements (see HUG-STR-003 \footref{url:baseline_report}).\\ \hline

Cost & Variable ports in HUG-CFG-304 substantially increase design complexity, as well as development, maintenance, and repair costs, compared to configurations without ports. Static canard ports have a similar effect, although to a lesser degree.
%The same reasoning can be applied to retractable landing gear mechanisms for HUG-CFG-302, HUG-CFG-304 due to the additional podded complexity. 
Moreover, the design of HUGO is subject to specific cost requirements (see: HUG-FIN-001 \footref{url:baseline_report}). \\ \hline

Performance & Complex design options featured are expected to result in higher design weights, which affect performance and fuel consumption. Moreover, the design of HUGO is subject to specific performance requirements (see: HUG-OPS-005, HUG-OPS-006 \footref{url:baseline_report}). %The type of landing gear was also part of the grading process, as it was analysed in \autoref{sec:Landing_gear_config}, HUG-CFG-302 and HUG-CFG-304 scoring lower due to the podded added drag.
\\ \hline

Sustainability & Sustainability is a defined project objective (HUG-OPS-008 \footref{url:baseline_report}). While variable wing ports and additional canard ports increase material usage and require more complex manufacturing processes, the primary drivers of emissions remain fuel consumption during operation and the production of sustainable aviation fuel (SAF).
\\
...
The preliminary configurations and materials shall be evaluated and assigned a value ranging from 1 to 5 in each trade-off criterion. The definition of specific scores for different criteria can be found in \autoref{ch:app_trade-off_rationale}. \autoref{tab:trade-off_scoring} presents the rationale used to quantify the scores for each criterion across the different configurations.

\begin{longtable}{|p{2.5cm}|p{13.5cm}|}
\caption{Trade-Off scoring for HUGO configurations}
\label{tab:trade-off_scoring} \\

\hline
\textbf{Criterion} & \textbf{Rationale for scoring} \\ 
\hline
\endfirsthead

\hline
\textbf{Criterion} & \textbf{Rationale for assigning scores} \\ 
\hline
\endhead
...
Versatility & Versatility is quantified as the range of experiments and stability conditions that can be tested with a given configuration. A low-wing configuration that excludes the evaluation of truss-braced wings receives a deduction of 2 points from the maximum score of 5, as this capability is a stakeholder requirement from NLR. A configuration that does not support canard-dependent designs receives a 2-point deduction, as it significantly restricts the available test envelope. Similarly, an inability to integrate variable wings results in a 1-point deduction. \\ \hline

Performance & Performance is evaluated based on how effectively the maximum Mach number at test altitude can be achieved while carrying the full instrumentation payload, with minimal drag and fuel consumption. Configurations that are lighter, less complex, and feature fewer protruding components will receive higher scores. \\ \hline

Reliability & Reliability is evaluated as the likelihood that the testbed successfully performs its intended function without failure under specified operating conditions throughout its lifetime. Mainframe fatigue at modular interfaces is identified as a primary reliability driver. A variable wing port incurs a 2-point deduction due to increased failure risk associated with its higher complexity, as well as the added aerodynamic panels required to cover unused mounting locations. A static canard port results in a 1-point deduction, as it introduces an additional potential failure point. \\ \hline

Cost & Cost is quantified as the manufacturing effort, materials, development, and maintenance required for each configuration. More complex and heavier designs will score lower in this category. Variable ports substantially increase design complexity as well as development, maintenance, and repair costs compared to configurations without ports, while static canard ports have a similar but less severe effect. \\ \hline
...
\begin{comment}
\begin{itemize}
    \item \textbf{Versatility} is quantified as the range of experiments and stability conditions that can be tested with the given configuration. A low-wing configuration that excludes the evaluation of truss-braced wings receives a deduction of 2 points from the maximum score of 5, as this capability is a stakeholder requirement from NLR. Similarly, a configuration that does not support canard-dependent designs receives a 2-point deduction. An inability to integrate variable wings results in a 1-point deduction, as it restricts the available test envelope.
    ...
    \item \textbf{Reliability} is evaluated as the likelihood that the testbed successfully performs its intended function without failure under specified operating conditions throughout its lifetime. Mainframe fatigue at modular interfaces identified as primary reliability drivers. A variable wing port incurs a 2-point deduction due to increased failure risk associated with its higher complexity, as well as the added aerodynamic panels required to cover unused mounting locations. A static canard port results in a 1-point deduction, as it introduces an additional potential failure point.
    ...
    \item \textbf{Sustainability} As mentioned in \autoref{sec:Environmental sustainability}, the primary contributors to emissions are fuel consumption and SAF production. While manufacturing complexity and material consumption of the configurations may also be considered, configurations with the highest fuel mass are expected to have the lowest overall performance in terms of sustainability.
\end{itemize}
\end{comment}
...
Performance & $2$ &
If a configuration cannot effectively achieve the required test conditions, the resulting test data has limited value. Therefore, moderate penalties in drag or weight are considered acceptable.
\\ \hline
...
Although performance was initially considered a viable trade-off criterion, preliminary mass and drag estimates and fuel fraction data obtained from the parametric model showed comparable values across all configurations as can be seen in \autoref{tab:performance_params}. Therefore, performance was excluded from the trade-off analysis.

% \todo{\textbf{CHECK VALUES IN TABLE!}}
\begin{table}[H]
\centering
\caption{Performance parameters for configurations}
\label{tab:performance_params}

\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\textbf{ID} & \textbf{MTOM [kg]} & \textbf{T/W} & \textbf{Fuel Fraction} & \textbf{CD0 Cruise} & \textbf{CD0 Mach Max} & \textbf{CD0 Go Around} & \textbf{CD0 Takeoff} \\
\hline\hline
HUG-CFG-301 & 37.728 & 0.346 & 0.273 & 0.0232 & 0.0227 & 0.0233 & 0.1896 \\\hline
HUG-CFG-302 & 35.736 & 0.362 & 0.276 & 0.0241 & 0.0237 & 0.0243 & 0.2874 \\\hline
HUG-CFG-303 & 37.683 & 0.345 & 0.273 & 0.0231 & 0.0226 & 0.0232 & 0.2724 \\\hline
HUG-CFG-304 & 38.085 & 0.341 & 0.272 & 0.0229 & 0.0224 & 0.0230 & 0.2695 \\\hline
%HUG-CFG-305 & 38.139 & 0.333 & 0.268 & 0.022 & 0.0219 & 0.022 & 0.214 \\
\end{tabular}%
}
\end{table}
...
The AHP was applied as described by Pant et al. \cite{Pant2022}. This process encompasses pairwise comparison of the trade-off criteria. To form a pairwise comparison matrix (PCM) for $n$ distinct criteria, $n(n-1)/2$ comparisons are required. The diagonal entries of the PCM are all equal to 1, and the remaining values are reciprocal pairs of the $n(n-1)/2$ comparison values.
...
\hline\hline
Cost           & 1 & $1/6$ & $1/4$ & 2  \\\hline
Versatility    & 6 & 1 & 3 & 7 \\\hline
Reliability    & 4 & $1/3$ & 1 & 4 \\\hline
Sustainability & $1/2$ & $1/7$ & $1/4$ & 1 \\\hline
\end{tabular}
\label{tab:pcm}
\end{table}

The next step is computing the normalised principal eigenvector (priority vector) \textbf{w} \footnote{\href{https://people.revoledu.com/kardi/tutorial/AHP}{Analytic Hierarchy Process (AHP) Tutorial} (accessed on 11.05.2026)}. The priority vector shows relative weights among the compared parameters; the higher the entry value, the higher the importance of its respective criterion.  Individual components of \textbf{w} can obtained according to \autoref{eq:priority_vector} for a matrix with $n$ criteria. The normalised PCM and resultant \textbf{w} are shown in \autoref{tab:pcm_norm}.
\begin{equation}
...
Cost           & 0.0870 & 0.1014 & 0.0566 & 0.1429 & 0.0967 \\\hline
Versatility    & 0.5217 & 0.6087 & 0.6667 & 0.5000 & 0.5743 \\\hline
Reliability    & 0.3478 & 0.2029 & 0.2222 & 0.2857 & 0.2647 \\\hline
Sustainability & 0.0435 & 0.0870 & 0.0556 & 0.0714 & 0.0644 \\\hline
...
\begin{equation}
\label{eq:cost_index}
    CI = \frac{\lambda_\text{max}-n}{n-1}\approx0.0492
\end{equation}
...
The clear overall winner is \textbf{HUG-CFG-303}, achieving the highest total score of \textbf{3.84}. HUG-CFG-304 is eliminated early in the evaluation due to failing to meet the minimum acceptable threshold in cost.

From a cost and manufacturability perspective, HUG-CFG-302 performs best due to its relatively low structural and production complexity. HUG-CFG-301 and HUG-CFG-303 also perform well in this category, as they include only an additional static canard port. In contrast, HUG-CFG-304 features a higher degree of modularity, which substantially increases both design and manufacturing complexity.

Regarding versatility, the configurations with higher modularity generally perform better, as they allow for a wider range of testing options. As explained in \autoref{tab:trade-off_scoring}, HUG-CFG-301 and HUG-CFG-302 each receive a 3-point penalty, as the former excludes testing of truss-braced wing designs and variable wings, while the latter excludes canard-based designs and variable wings. Similarly, HUG-CFG-303 receives a 1-point deduction due to the absence of a variable wing port. In contrast, the fourth configuration includes full modularity and therefore achieves the maximum score of 5.

Reliability is inversely correlated with system complexity. Consequently, configurations with variable ports and increased modularity tend to perform worse in this category. HUG-CFG-304 receives the lowest reliability score, with a 2-point penalty for the variable wing port and the associated aerodynamic panels, and a further 1-point penalty for the static canard port, as specified in \autoref{tab:trade-off_scoring}. Similarly, HUG-CFG-301 and HUG-CFG-303 each receive a 1-point deduction due to the additional failure mode introduced by the static canard port.

Finally, sustainability is assessed based on fuel mass predictions obtained from the parametric model. Although it was previously stated that the fuel fractions are comparable across all configurations, fuel burn and SAF production are still taken into account, as they are the primary contributors to emissions. The fuel consumption values for all five configurations are presented in \autoref{tab:fuel_consumption}.
...
\cline{2-5}
\multicolumn{1}{c|}{} 
& \textbf{HUG-CFG-301} & \textbf{HUG-CFG-302} & \textbf{HUG-CFG-303} & \textbf{HUG-CFG-304}\\
\hline
\textbf{Fuel Mass} & 10.29 kg & 9.87 kg & 10.26 kg & 10.34 kg\\
\hline
\end{tabular}

\end{table}

In terms of fuel efficiency, HUG-CFG-302 performs best with the lowest fuel mass. The other three configurations exhibit the highest fuel consumption, with differences of approximately 0.5 kg relative to the most efficient design, and therefore receive the lowest sustainability scores. This is further attributed to the simpler manufacturing process and lower material usage of configuration 2, whereas the alternatives would require more complex manufacturing and greater material usage, particularly for reinforcing the variable and static ports.
...
The landing gear is a retractable configuration to minimise aerodynamic drag at high subsonic Mach numbers. During the final design, a selection will be made between buried retractable landing gear  inside the fuselage or buried side podded landing gear.
