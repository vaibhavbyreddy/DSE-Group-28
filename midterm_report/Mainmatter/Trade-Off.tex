...
\begin{figure}[H]
    ...
    \caption{Initial HUGO configuration concepts}
    \label{fig:hugo-configurations-303-304}
\end{figure}
...
\todo{The below justifications will not be included in the final version, currently just for overview of justifications}
\textbf{Versatility}:
\begin{enumerate}
    \item HUG-CFG-301: Does not allow for testing of truss based wing - scores 2 - eliminated because of versatility no-go value.
    \item HUG-CFG-302: No canard port - scores 3
    \item HUG-CFG-303: Wing and canard static ports- scores 4.
    \item HUG-CFG-304: Full range of configurations testable - scores 5
    \item HUG-CFG-305: Same as 4 - scores 5.
\end{enumerate}

\textbf{Reliability}:
\begin{enumerate}
    \item HUG-CFG-301:  Variable wing port introduces moderate fatigue exposure; low wing configuration provides good accessibility for timely intervention — moderate failure risk — score 3
    \item HUG-CFG-302: Static port results in minimal fatigue exposure; side podded landing gear ensures good accessibility enabling routine preventive maintenance — no anticipated failure — score 5
    \item HUG-CFG-303: Static ports result in minimal fatigue exposure; buried landing gear limits accessibility, though preventive maintenance remains feasible with planning — soft failure risk — score 4
    \item HUG-CFG-304: Variable wing port introduces moderate fatigue exposure; side podded landing gear ensures good accessibility for timely intervention — moderate failure risk — score 3
    \item HUG-CFG-305: Variable wing port introduces moderate fatigue exposure; buried landing gear limits accessibility, increasing the likelihood of undetected degradation — severe failure risk — score 2 — eliminated
\end{enumerate}

\textbf{Cost}
almost the same as reliability, but differs for config 3 due to the complexity of side podded LG; low wing is less complex than high from a structural point of view

\textbf{Performance}
same for all

\textbf{Sustainability}
based on fuel mass, most important, redo appendix
