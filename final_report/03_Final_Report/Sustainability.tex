...
The main environmental benefits of this project would be the research the platform would enable. One of the main focuses of research for HUGO would be the study of high aspect ratio wings. Almost 80 \% of drag is caused by lift induced drag. This can be alleviated by increasing the aspect ratio of the wing (insert citation). Modern airliners has already introduced methods to artificiality increasing the aspect ratio such as foldable wing-tips and winglets but the current industry estimate is a further increase in aspect ration (insert citation here). The increase the aspect ratio  drastically changes the behaviour of the wing change with one of the major areas affected being aeroelastic behaviour of the wing. The addition of structural re-enforcement in the wing leads to more weight which in turn increases the fuel consumption and in some instances renders the concept infeasible. As wings with a higher aspect ration is more susceptible to (list aeroelastic phenomenon). The weight penalty for the additional loads. Aeroelastic phenomenon such as flutter. There are two major benefits by understanding aeroelastic behaviour: firstly, it enables the implementation of much higher aspect ratio wing and secondly it enables the design of control-systems which can alleviate the loads thereby potentially lead to weight-reduction (as less structural reinforcement is necessary).  
...
Another benefit of the versatility of the design is the potential to reduce the development time for fuel-saving technologies. Instead of spending time on designing an entirely new platform.  Finally the type of research enabled by HUGO will more accurately predict aeroelastic behaviour and other phenomenon which is difficult to model in a windtunnel (insert citation). These factors can lead to a shorter development time and thereby leading to a faster utilization of novel concepts. A fast implementation will lead to a larger overall reduction in aviation emissions.  
