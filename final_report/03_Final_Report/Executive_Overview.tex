...
Project HUGO disrupts the status quo by offering a certified, reconfigurable testbed that operates on Sustainable Aviation Fuel (SAF) to minimize altitude emissions \cite{Gaillot2023}. At a targeted commercial rate of €25,000 per flight day, HUGO significantly reduces the programmatic costs of aeroelastic research while delivering high-fidelity, free-flight data.
...
\begin{itemize}
    ...
    \item \textbf{Flight Test Instrumentation:} As a flying laboratory, HUGO's value relies on high-fidelity data acquisition. The standardized payload suite includes dual LIDAR sensors for in-flight wing deflection measurement, 16 strain gauges placed on various points on the wing skin, and a DJI O4 camera system to visually monitor aeroelastic behaviour. Additionally, the wing-to-fuselage structural link is formed by four 6-axis load cells, isolating and measuring aerodynamic forces on the wing. This is in addition to standard avionics sensors such as a 6 axis Inertial measurement unit (IMU), Global Positioning System (GPS) reciever and gyroscope. HUGO also has space to accomodate an additional 5kg of payload in a 5 liter volume, to house test equipment or additional sensors based on the client's needs. The masses and positioning of these components were done in a preliminary 3D model. Doing so allowed us to accurately estimate the CG of the aircraft, as well as wing positioning, and empennage sizing.
...
The material selection and MAI strategy for HUGO are closely related. The chosen manufacturing processes directly dictate the final mechanical properties, add constraints on design decisions and economic feasibility of the product. Given HUGO's intended production scale (an initial fleet of five aircraft), the design heavily prioritizes out-of-autoclave (OOA) processes to minimize tooling infrastructure costs while maintaining the structural integrity required for Mach 0.75 aeroelastic testing.
...
\section{Operations, Certification, and Safety}
...
For any engineering project it is vital to understand how it directly affects its surroundings. This can be done  by looking at the noise and emissions generated by the  flight of the aircraft. By stipulating a maximum noise-level at 90 db(A) it is possible to have an upper bound for the noise. This number was chosen based on the general guidelines for jet-aircraft in ICAO Annex 16. After performing the analysis the maximum noise was found to be 87.5 dB(A) thereby with this upper bound. Using emission data for the burning of Jet-A fuel it was possible to identify the different compounds which makes up the emission. Then evaluating each of the compounds based on the Global Warming Potential (GWP) of 1 kg of burned fuel to be 10.923 GWP. This emission model would be used in the LCA to estimate the impact of the operation. 
