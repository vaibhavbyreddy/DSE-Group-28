...
\begin{itemize}
    ...
    \item $t/c$: The range encompasses thin and potentially unstable wings and high-thickness profiles similar to those at wing roots of large aircraft. As for the critical points, parasitic drag increases with $t/c$, but the wing weight decreases significantly. Thus, three values across the parameter range are selected for evaluation.
    \item $C_{l_\text{max}}$: To ensure sufficient low-speed performance, a minimum value of this parameter is established.
    \item $C_{m_{ac}}$: The extrema values of this parameter are most constraining for sizing the empennage for controllability and, as such, taken as critical points. The minimum $C_{m_{ac}}$ accounts for high camber sections such as NACA 4415 with $C_{m_{ac}}$ of around -0.12. \footnote{\label{foot:NACA_4415}\href{http://airfoiltools.com/airfoil/details?airfoil=naca4415-il}{Airfoil Tools, NACA 4415}}, while the upper bound accounts for reflexed airfoils such as Eppler E184 \footnote{\label{foot:Eppler_E184}\href{http://airfoiltools.com/airfoil/details?airfoil=e184-il}{Airfoil Tools, Eppler E184}} with $C_{m_{ac}}$ of -0.02.
    \item $C_{l_0}$: The impact of this parameter is secondary - provided $C_{l_\alpha}$ and $C_{l_{\mathrm{max}}}$ are satisfactory, it only changes the angle of attack required for a given amount of lift. Hence, values above or close to zero are allowed, with zero taken as the critical value.
...
\begin{table}[]
    ...
    \begin{tabular}{|c|c|c||c|c|c|}\hline
         ...
         $C_{l_{0}}$ & [0, $\infty$)  & 0 & $C_{l_{\alpha}}$ & $[\sim2\pi,\ \infty)$ & 2$\pi$ \\\hline
...
The geometry of the standard planform, to be used when the client does not provide one of their own, is presented in \autoref{tab:standard_wing_planform}. The planform is based on the one obtained in the Midterm Report \footnote{Midterm Report}. For safety reasons, it shall be ensured that the standard planform does not experience aeroelastic instability in all operating conditions up to Mach 0.75. This is evaluated in \todo{autoref}.
