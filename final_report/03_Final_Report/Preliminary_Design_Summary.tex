...
\section{Final LCA}
\label{sec:finalLCA}
Now having the system, subsystem sizing, emission-calculator (see \autoref{sec:emission_analysis}) and detailed flight profile it is possible the run and evaluate the LCA. 
The model used for evaluating the emissions was (IPCC) 2021
(ISO 14067) which is a standard method used for evaluating the impact of processes.  
The LCA is consisting of 4343 individual processes and with 52547 process links. Therefore the diagram which can be found at (insert reference here) is just a small section of it. The full LCA can be found at (insert link). 
...
This an overview of the expected expenses for all post-DSE activities. The cost-break down structure identifies each cost element and links it to a larger group. The diagram is of the tree structure with the total cost in the top and then the cost of elements branching out underneath. A detailed cost break down was already performed in the market analysis to ascertain the profitability of the project \autoref{ch:market_anaysis}. This was used to make the diagram which can be found in \autoref{fig:proj_design}, and the corresponding post-DSE Gantt Chart in \autoref{fig:gantt_chart}
